...
const PAGES = [
  { src: "/oss-brief-1.jpg", desc: "Cover — Matrix idle lock · S1R1US Labs open source brief" },
  { src: "/oss-brief-2.jpg", desc: "Open source brief page 2 · S1R1US Labs" },
  { src: "/oss-brief-3.jpg", desc: "Open source brief page 3 · S1R1US Labs" },
];
...
function ImageViewer() {
  return (
    <div className="space-y-4">
      {PAGES.map((p) => (
        <SeoImage key={p.src} src={p.src} desc={p.desc} className="w-full rounded-md border border-rule bg-surface" />
      ))}
    </div>
  );
}
