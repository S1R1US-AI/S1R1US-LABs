...
function hostOf(msg: string) {
  const m = msg.match(/([a-z0-9.-]+\.[a-z]{2,})/i);
  return m?.[1]?.toLowerCase() ?? "desk";
}

/** Honest classify. Never green a host because we have a story — only if the miss is understood AND not core. */
export function classifyFail(msg: string): { resolved: boolean; attention: boolean; verdict: string } {
  const m = msg.toLowerCase();
  if (/fapi\.binance|binance\.com/.test(m) && /451|403|geo/.test(m)) {
    return {
      resolved: true,
      attention: false,
  ...
  if (/bybit/.test(m) && /403|forbidden/.test(m)) {
    return {
      resolved: true,
      attention: false,
  ...
  if (/coingecko/.test(m) && /429/.test(m)) {
    return {
      resolved: true,
      attention: false,
...
export function recordDeskFails(fails: string[], at = new Date().toISOString()) {
  ...
  for (const msg of fails) {
    ...
    if (last && Date.now() - Date.parse(last.at) < 6 * 60_000) {
      last.at = at;
...
export function listDeskErrors(): DeskError[] {
  return RING.slice(0, MAX);
}

export function attentionErrors(): DeskError[] {
  return RING.filter((r) => r.attention || !r.resolved).slice(0, 40);
}