...
function q(quotes: Quote[], symbol: string): Quote | undefined {
...
function usdM(n: number | null | undefined): string {
  if (n == null || !Number.isFinite(n)) return "n/a";
  const sign = n >= 0 ? "+" : "-";
  return `${sign}$${(Math.abs(n) / 1e6).toFixed(0)}M`;
}

function goldBtcVsMedian(snap: DeskSnapshot): { oz: number | null; vsMed: number | null; med: number | null } {
  const oz = snap.goldBtc.ozPerBtc;
  const series = snap.goldBtc.series.map((p) => p.ozPerBtc).filter((n) => Number.isFinite(n) && n > 0);
  if (oz == null || !Number.isFinite(oz) || series.length < 8) return { oz, vsMed: null, med: null };
  const sorted = [...series].sort((a, b) => a - b);
  const med = sorted[Math.floor(sorted.length / 2)]!;
  return { oz, vsMed: (oz - med) / med, med };
}

const BTC_BUY_RE = /bitcoin|btc|purchas|acquir|treasury|addition/i;
const DILUTE_RE = /atm|at-the-market|offering|convertible|dilut|note offering|senior notes/i;
const ROTATION_RE =
  /rotat(?:e|ion)|into bitcoin|into btc|debasement|digital gold|gold.{0,28}bitcoin|bitcoin.{0,28}gold|nasdaq.{0,40}bitcoin|ai bubble|401\s*\(?k\)?|endowment|strategic bitcoin reserve|bitcoin reserve|insurance.{0,20}bitcoin|etf inflow|whale.{0,20}(buy|accumul)/i;

export function runBots(snap: DeskSnapshot): BotBrief[] {
  const live = (id: Exclude<BotId, "helios">, summary: string, bullets: string[], stance: Stance): BotBrief => ({
    id,
    ...roster(id),
    status: "live",
    summary,
  ...
  const mag7 = MAG7_TICKERS.map((s) => q(snap.quotes, s)).filter(Boolean) as Quote[];
  const mag7Avg =
    mag7.length > 0 ? mag7.reduce((s, x) => s + (x.changePct ?? 0), 0) / mag7.length : null;
  ...
  const filings = live(
    "filings",
    buys.length
      ? `${buys.length} BTC-treasury event(s) in the DAT book — demand, not equity beta.`
      : dilutes.length
        ? `${dilutes.length} ATM/offering print(s) — paper supply, not a BTC bid.`
        : material.length
          ? `${material.length} material filing(s), none read as a BTC purchase.`
          : "No 8-K / 10-Q / 10-K in MSTR · COIN · MARA · SMLR.",
    [
    ...
    buys.length >= 1 ? "ACCUMULATE" : dilutes.length >= 1 ? "WAIT" : "HOLD",
  );

  const etfFlow = snap.capital.etfFlow;
  const datBtc = snap.capital.dats.reduce((s, d) => s + (d.btc ?? 0), 0);
  const flowOut = etfFlow != null && etfFlow < -50_000_000;
  const flowFomo = etfFlow != null && etfFlow > 400_000_000;
  const earnings = live(
    "earnings",
    etfFlow != null
      ? `BTC demand: US spot ETF ${usdM(etfFlow)} last session. DAT book ${datBtc.toLocaleString("en-US", { maximumFractionDigits: 0 })} BTC. IBIT ${fmtPct(ibit?.changePct)} vs Coinbase ${fmtPct(snap.btc.changePct)}.`
      : `ETF flow missing this cycle. DAT book ${datBtc.toLocaleString("en-US", { maximumFractionDigits: 0 })} BTC. IBIT ${fmtPct(ibit?.changePct)}.`,
    [
      ...
      `Label only, not a vote: ${["MSTR", ...MINER_TICKERS, "NVDA"].map((s) => `${s} ${fmtPct(q(snap.quotes, s)?.changePct)}`).join(" · ")}`,
    ],
    flowOut ? "ACCUMULATE" : flowFomo ? "WAIT" : "HOLD",
  );

  const btcLagGold = (ibit?.changePct ?? 0) - (gld?.changePct ?? 0) < -1.2;
  const riskOff = (spy?.changePct ?? 0) < -1 && (dxy?.changePct ?? 0) > 0.3;
  const gb = goldBtcVsMedian(snap);
  const ratioCheap = gb.vsMed != null && gb.vsMed <= -0.08;
  const ratioRich = gb.vsMed != null && gb.vsMed >= 0.12;
  const chaseMetals =
    (gld?.changePct ?? 0) > 0.4 && (slv?.changePct ?? 0) > 0.5 && (ibit?.changePct ?? 0) > 2;
  const sector = live(
    "sector",
    `RV: IBIT ${fmtPct(ibit?.changePct)} vs GLD ${fmtPct(gld?.changePct)}. BTC ${gb.oz != null ? `${gb.oz.toFixed(1)} oz` : "n/a"} gold (${gb.vsMed != null ? fmtPct(gb.vsMed * 100) : "n/a"} vs 1y median). DXY ${fmtPct(dxy?.changePct)} SPY ${fmtPct(spy?.changePct)}.`,
    [
      ibit && gld
        ? `Session vote: IBIT minus GLD = ${fmtPct((ibit.changePct ?? 0) - (gld.changePct ?? 0))}`
        : "IBIT/GLD incomplete",
      gb.vsMed != null
        ? `Valuation vote: BTC/gold ${ratioCheap ? "cheap (accumulate)" : ratioRich ? "rich (wait)" : "near median"} · ${gb.oz?.toFixed(1)} oz vs med ${gb.med?.toFixed(1)}`
        : "BTC/gold 1y median incomplete",
    ...
    chaseMetals
      ? "WAIT"
      : ratioCheap || btcLagGold || (riskOff && (ibit?.changePct ?? snap.btc.changePct ?? 0) < 0)
        ? "ACCUMULATE"
        : ratioRich
          ? "WAIT"
          : "HOLD",
  ...
  const sentiment = live(
    "sentiment",
    fg
      ? `Fear & Greed ${fg.value} (${fg.label}). Headlines are context, not a vote.`
      : `${snap.headlines.length} headlines. Fear & Greed missing — no mood vote.`,
    [
      ...(fg ? [`Alternative.me ${fg.value} — ${fg.label} (vote: ≤30 accumulate, ≥70 wait)`] : ["F&G missing"]),
      ...snap.headlines.slice(0, 3).map((h) => `Label: ${h.source}: ${h.title}`),
    ],
    fear ? "ACCUMULATE" : greed ? "WAIT" : "HOLD",
  );
  ...
  const btcBid = ibitCh > 0.25;
  const fromNasdaq = btcBid && qqqCh != null && ibitCh - qqqCh >= 1.4;
  const fromAi = btcBid && nvdaCh != null && ibitCh - nvdaCh >= 1.8;
  const fromGoldPaper = btcBid && gldCh != null && ibitCh - gldCh >= 1.4 && gldCh < 0.25;
  const fromCre = btcBid && vnqCh != null && ibitCh - vnqCh >= 1.4 && vnqCh < 0.3;
  const intoNasdaq =
    qqqCh != null && qqqCh > 1 && ibitCh - qqqCh <= -1.6 && ibitCh < 0.2;
  const rotHits = [fromNasdaq, fromAi, fromGoldPaper, fromCre].filter(Boolean).length;
  ...
  const rotation = live(
    ...
    rotationIn
      ? `Capital rotating into BTC: ${[
          ...
          .filter(Boolean)
          .join(" · ")}. Whale net ${usdM(whaleNet)}.`
      : rotationOut
        ? `Capital leaving BTC into Nasdaq (QQQ ${fmtPct(qqqCh)} vs IBIT ${fmtPct(ibitCh)}). Do not chase AI.`
        : `No confirmed rotation. IBIT ${fmtPct(ibitCh)} QQQ ${fmtPct(qqqCh)} NVDA ${fmtPct(nvdaCh)} GLD ${fmtPct(gldCh)}. Whale net ${usdM(whaleNet)}. BTC.D ${snap.onchain.btcDom != null ? `${snap.onchain.btcDom.toFixed(1)}%` : "n/a"}.`,
    [
      ...
      rotNews[0]
        ? `Copy: ${rotNews[0].source}: ${rotNews[0].title}`
        : "No rotation copy in free RSS this cycle.",
      forms4.length ? `Label — ${forms4.length} Form 3/4/5 (does not vote).` : "Form 4 label quiet.",
    ],
    rotationIn ? "ACCUMULATE" : rotationOut ? "WAIT" : "HOLD",
  );

  const lanes: { id: BotId; lane: string; brief: BotBrief }[] = [
    { id: "filings", lane: "event", brief: filings },
    { id: "earnings", lane: "flow", brief: earnings },
    { id: "sector", lane: "rv", brief: sector },
    { id: "sentiment", lane: "mood", brief: sentiment },
    { id: "rotation", lane: "rotation", brief: rotation },
  ];
  ...
  const coordinator = live(
    "coordinator",
    twoSource
      ? `Orthogonal confirm: ${buyLanes.join(" + ")} (${buyLanes.length} lanes).`
      : `No two-lane confirm. Buy lanes: ${buyLanes.join(", ") || "none"}. Wait lanes: ${waitLanes.join(", ") || "none"}.`,
    [
  ...
  return [filings, earnings, sector, sentiment, rotation, coordinator];
}

export function heliosCall(snap: DeskSnapshot, briefs: BotBrief[], navUsd: number): HeliosCall {
  ...
  const longsPaying = funding != null && funding > 0.0001;
  const shortsPaying = funding != null && funding < -0.0001;
  ...
  const ratioCheap = gb.vsMed != null && gb.vsMed <= -0.08;
  const ratioRich = gb.vsMed != null && gb.vsMed >= 0.12;
  const etfMelt = etfFlow != null && etfFlow > 350_000_000 && (rsi == null || rsi >= 50);
  ...
  const checks = [
    ...
    { label: "RSI(14) hourly < 45", pass: rsi != null && rsi < 45 },
    { label: "Fear & Greed ≤ 55", pass: fg <= 55 },
    { label: "Long/short not crowded (>1.6, OI-weighted)", pass: ls == null || ls < 1.6 },
    { label: "Funding not overheated (>0.05%, longs paying)", pass: funding == null || funding < 0.0005 },
    {
      label:
        sellWall != null && buyWall != null && buyWall > 0
          ? `Sell wall not 2× buy wall near last (now ${(sellWall / buyWall).toFixed(1)}×)`
          : "Sell wall not 2× buy wall near last",
      pass: sellWall == null || buyWall == null || sellWall < buyWall * 2,
    },
    { label: "Kimchi premium not crowded (>3%)", pass: kimchi == null || kimchi < 3 },
    { label: "CNY OTC USDT not overheating (>2%)", pass: cnyPrem == null || cnyPrem < 2 },
    { label: "EM tape not crowded (3+ regions hot)", pass: emHot < 3 },
    { label: "ETF session not a melt-up", pass: !etfMelt },
  ...
  const thesis = [
    ...
    macd50 || macd200
      ? `MACD-50 hist ${macd50 ? macd50.hist.toFixed(1) : "n/a"} · MACD-200 hist ${macd200 ? macd200.hist.toFixed(1) : "n/a"}${macdDiscount ? " — histogram discount, accumulate." : "."}`
      : "",
    ...
    funding != null
      ? `Funding ${(funding * 100).toFixed(4)}% — ${longsPaying ? "longs paying (crowded longs losing carry)" : shortsPaying ? "shorts paying (shorts losing carry)" : "flat carry"}.`
      : "",
    sellWall != null || buyWall != null
      ? `Walls sell ${sellWall != null ? usdM(sellWall) : "n/a"} vs buy ${buyWall != null ? usdM(buyWall) : "n/a"} (${wallBias}).`
      : "",
    ...(snap.positioning.venues ?? []).map((v) =>
      `${v.name} LS ${v.longShort != null ? v.longShort.toFixed(2) : "n/a"} fund ${v.fundingRate != null ? `${(v.fundingRate * 100).toFixed(3)}%` : "n/a"} OI ${v.openInterestUsd != null ? usdM(v.openInterestUsd) : "n/a"}.`,
    ),
    etfFlow != null ? `US spot ETF ${usdM(etfFlow)}.` : "",
    cheapVsGold ? "IBIT lagging gold this session — BTC cheap vs metal." : "",
    gb.vsMed != null
      ? `BTC/gold ${gb.oz?.toFixed(1)} oz (1y median ${gb.med?.toFixed(1)}, ${fmtPct(gb.vsMed * 100)})${ratioCheap ? " — cheap, accumulate." : ratioRich ? " — rich vs gold, do not chase." : "."}`
      : "",
    `Session ${asia.session}.`,
    kimchi != null ? `Upbit kimchi ${kimchi >= 0 ? "+" : ""}${kimchi.toFixed(2)}% vs Coinbase.` : "Kimchi unavailable.",
    asia.hkPremiumPct != null
      ? `HashKey HK ${asia.hkPremiumPct >= 0 ? "+" : ""}${asia.hkPremiumPct.toFixed(2)}%.`
      : "",
    asia.cnyOtc.usdtCny != null
      ? `CNY OTC USDT ${asia.cnyOtc.usdtCny.toFixed(3)} vs FX ${asia.cnyOtc.officialCny?.toFixed(3) ?? "n/a"} (${asia.cnyOtc.premiumPct != null ? `${asia.cnyOtc.premiumPct >= 0 ? "+" : ""}${asia.cnyOtc.premiumPct.toFixed(2)}%` : "n/a"}).`
      : "CNY OTC unavailable.",
    ...
    ...em.regions.map(
      (r) =>
        `${r.name} ${r.flow}${r.premiumPct != null ? ` ${r.premiumPct >= 0 ? "+" : ""}${r.premiumPct.toFixed(1)}%` : ""}.`,
    ),
    rot
  ...
  const cli =
    stance === "BUY" || stance === "ACCUMULATE"
      ? `coinbase orders preview --dry-run product_id=BTC-USD side=BUY type=market quote_size=${Math.max(clipUsd, 10)}`
  ...
  return { stance, conviction, clipUsd, brief, thesis, checks, cli, preview };
}
