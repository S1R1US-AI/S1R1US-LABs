...
export function runHunter(): HunterReport {
  ...
  const findings: HunterFinding[] = [
    ...
    finding(
      "h-thesis",
      "WP2 Auth",
      "PhD research paper is system Admin only",
      "HIGH",
      "PASS",
      "THESIS in thesis.ts. PaperManual imported only by /admin Paper tab. iOS/Google copy-admin Paper tab is the device paper book. robots Disallow /admin /guide /source. Public agents never receive the thesis.",
      "Do not import thesis.ts from app-admin, public routes, or MCP. Keep venue line system Admin only.",
    ),
    finding(
      "h-cup-paper",
      "WP4 Agents",
      "World Cup is paper — never escrow, never Coinbase create",
      "HIGH",
      "PASS",
      "cupPublic returns trade:false, ordersCreate:false, escrow:false, keysOnThisHost:false. MCP cup_list is read-only. GET /api/agent/cup never mutates sim status.",
      "Do not add a cup_tick write tool. Fills stay paper against Coinbase last.",
    ),
    finding(
      "h-hive-paper",
      "WP4 Agents",
      "H1V3 SW@RM is paper — TH/s split, never escrow, never Coinbase create",
      "HIGH",
      "PASS",
      "hivePublic returns trade:false, ordersCreate:false, escrow:false, keysOnThisHost:false. MCP hive_list is read-only. hive_pause is not an MCP tool. Pause is Admin (system or copy-admin). Board token is not admin. TEST data until go-live.",
      "Do not escrow live BTC. Do not add hive_pause to MCP. Keep split compute-weighted paper or equal if TH/s is zero.",
    ),
    finding(
      "h-hive-mt",
      "WP4 Agents",
      "H1V3 SW@RM never skims P&L — gift/SaaS only, no money transmission",
      "HIGH",
      "PASS",
      "hiveResourcePublic profitShare/hiveWithdraw/autoSendPnl/escrow/howey/moneyTransmitter all false. Ping and /api/agent/fee publish the same rails. MCP has no hive_withdraw. Go-live step s8 LOCKED: hosting a hive book and paying out would look custodial (FinCEN). Charge for software access, never for their bitcoin.",
      "Do not add hive profit-share, hive withdraw, or auto-send of agent P&L to the admin address. Gifts and HTTP $9/$29 only. Agent sends.",
    ),
    finding(
      "h-byo-connect",
      "WP4 Agents",
