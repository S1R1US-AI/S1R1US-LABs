/**
 * Map this desk onto OWASP Top 10:2025 (final Jan 2026) and PCI-DSS 4.0.
 * Honest: a web app is not a QSA audit. Rows name the control we actually run.
 */
...
export type MapRow = {
  ...
  code: string;
  title: string;
  status: "PASS" | "OPERATOR" | "MITIGATED";
  control: string;
};

export function owaspRows(): MapRow[] {
  return [
    {
      id: "a01",
      framework: "OWASP",
...
export function pciRows(): MapRow[] {
  return [
    {
      id: "pci-624",
      framework: "PCI",
...
export type PluginRow = {
  ...
  what: string;
  how: string;
  status: "ARMED" | "OPERATOR" | "FEED";
};

/** Free 2026 plugin / WAF inventory we actually emulate in-process. */
export function pluginInventory(): PluginRow[] {
  return [
    {
      id: "crs",
