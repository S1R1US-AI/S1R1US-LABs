...
import { t as MCP_TOOLS } from "./agent-security-dJ5dmEbA.mjs";
import { s as GO_LIVE_STEPS } from "./go-live-CaLXlH4n.mjs";
import { p as mandatePublic } from "./mandate-Bm3-3k0n.mjs";
import { d as hiveResourcePublic } from "./hive-resource-BThKJHxr.mjs";
import { d as lockStatusView, t as LOCK_DEFAULT } from "./lock-status-CQ3k2p73.mjs";
import { i as LOCKED_FUNCTIONS } from "./oss-roadmap-BXfG3jrB.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/alignment-CDukfrXE.js
var alignment_CDukfrXE_exports = /* @__PURE__ */ __exportAll({
	n: () => alignment_exports,
	t: () => alignmentScore
});
/** Alignment Score 1–100: security protocols vs system mandate. Client-safe. */
var alignment_exports = /* @__PURE__ */ __exportAll$1({ alignmentScore: () => alignmentScore });
/** Keep in lockstep with launch/build.ts and launch/model.ts — relative-only so node:test can score. */
var LIVE_TRADES_LOCKED = true;
var PATH_A_MINT_LOCKED = true;
function grade(n) {
	if (n >= 90) return "A";
	if (n >= 80) return "B";
	if (n >= 70) return "C";
...
function check(id, family, label, pass, weight, proof) {
	return {
		id,
		family,
		label,
...
function alignmentScore(hunterReport) {
	...
	const termIdsOk = hunter?.findings ? hunter.findings.some((f) => f.id === "h-legal" && f.status === "PASS") : true;
	const hunterCritOpen = hunter?.findings?.filter((f) => f.status === "OPEN" && (f.severity === "CRITICAL" || f.severity === "HIGH")) ?? [];
	const hunterRailsHold = !MCP_TOOLS.has("orders_create") && !MCP_TOOLS.has("hive_withdraw") && !MCP_TOOLS.has("lock_set");
	...
	const checks = [
		...
		check("s-pred-paper", "legal", "S1R1US Pr3d1ctions is paper only — pred_arm is live-sim, never live funds", MCP_TOOLS.has("pred_list") && MCP_TOOLS.has("pred_bet") && MCP_TOOLS.has("pred_arm") && !MCP_TOOLS.has("pred_live") && !MCP_TOOLS.has("orders_create"), 3, "pred_list + pred_arm + pred_bet; no pred_live")
	];
	const totalWeight = checks.reduce((n, c) => n + c.weight, 0) || 1;
	const score = clampScore(checks.reduce((n, c) => n + (c.pass ? c.weight : 0), 0) / totalWeight * 100);
	const pass = checks.filter((c) => c.pass).length;
	const fail = checks.length - pass;
	const aligned = score >= 90 && fail === 0;
	const headline = aligned ? `ALIGNMENT SCORE ${score}/100 ${grade(score)} — security protocols and system mandate are aligned` : `ALIGNMENT SCORE ${score}/100 ${grade(score)} — ${fail} check(s) off mandate / protocol`;
	return {
...
//#endregion
export { alignment_CDukfrXE_exports as n, alignmentScore as t };
