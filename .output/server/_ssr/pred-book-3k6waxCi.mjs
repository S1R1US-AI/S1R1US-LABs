...
//#region node_modules/.nitro/vite/services/ssr/assets/pred-book-3k6waxCi.js
/** S1R1US Pr3d1ctions — paper Polymarket-style book. ph0 BTC only. Never escrow. Never a DCM. */
var PRED_PATH = "/pr3d";
var PRED_API = "/api/agent/pred";
var PRED_GRANT_USD = 42e3;
var PRED_FALLBACK_BTC = 105e3;
var PRED_MAX_BET_FRAC = .15;
var PRED_MIN_BET = 1e-4;
var PHO_ASSET = "ph0 BTC";
var PHO_WALLET = "Ph0 W@ll3t";
...
var PRED_START_PHO = grantPho(PRED_FALLBACK_BTC);
round8(PRED_START_PHO * PRED_MAX_BET_FRAC);
var OFFICIAL_ANALYSIS = {
	asOf: "2026-09-06",
	liveFunds: false,
	paper: true,
	heading: "Official docs analysis — what can go live on s1r1us.ai",
	verdict: "Public Polymarket Gamma and Kalshi market-list odds may be shown as labels. A paper Yes/No book in ph0 BTC may run for W1S3 0WL$, Admins, and AI agents. A licensed real-money S1R1US book is a LOCKED future goal (estimated 2027-06-01) for live AI agents and Admin users. Coinbase Wallet bets, Sparrow bets, Kalshi order routing, and Polymarket CLOB routing are NEVER on this host.",
	sources: [
		{
			id: "kalshi-dev-agree",
			name: "Kalshi Developer Agreement §3",
			url: "https://assets.kalshi.com/Kalshi-Developer-Agreement.pdf",
			finding: "Use of Kalshi APIs is expressly limited to facilitating a member's own trading. §3.1 forbids collecting, caching, aggregating, or sharing API data with third parties without written authorization. §3.2 forbids facilitating trading or account creation by other members. §3.7 forbids sublicensing.",
			verdict: "NEVER"
		},
		{
			id: "kalshi-docs",
			...
			finding: "docs.kalshi.com binds callers to the Developer Agreement. Help Center (help.kalshi.com/en/articles/13823854-kalshi-api) points members to that API for their own orders. Demo env is Kalshi's sandbox, not a third-party book. Broker access is FCM (e.g. Apex), not a website matching engine.",
			verdict: "NEVER"
		},
		{
			...
			finding: "Unauthenticated GET /markets on the public Trade API hosts can be read as labels (Yes %). That is display, not order routing. This host never copies Kalshi's book into a live wager.",
			verdict: "LIVE"
		},
		{
			id: "kalshi-dcm",
			...
			finding: "Kalshi members trade only for themselves and must not serve as an intermediary. Operating an event-contract book for others requires DCM/FCM registration.",
			verdict: "NEVER"
		},
		{
			id: "poly-gamma",
			...
			finding: "Gamma (gamma-api.polymarket.com) and Data API are public, no auth — market discovery and activity. Safe to display as education labels.",
			verdict: "LIVE"
		},
		{
			id: "poly-clob",
			...
			finding: "Order placement needs EIP-712 wallet auth plus HMAC L2 credentials. Builders route a user's own wallet to Polymarket's CLOB. That still makes this host a trading front-end. Polymarket US (QCX LLC) is a separate CFTC DCM.",
			verdict: "NEVER"
		},
		{
			id: "poly-geo",
			...
			finding: "GET https://polymarket.com/api/geoblock. International CLOB order placement is blocked for the United States and other jurisdictions. Builders must hard-block, not warn. VPN bypass is a ToS violation.",
			verdict: "NEVER"
		},
		{
			id: "ninth-circuit",
			...
			finding: "A federal DCM licence does not pre-empt state gambling law. Unlicensed live-funds books on a website/app remain unlawful.",
			verdict: "NEVER"
		},
		{
			id: "store-policy",
			...
			finding: "iOS and Google listings forbid real-money gaming. The education PWA may ship a paper book only.",
			verdict: "NEVER"
		},
		{
			id: "coinbase-wallet",
			...
			finding: "Coinbase Wallet is an EVM self-custody wallet. It is not a Kalshi member API and is not a CFTC DCM. Connecting it here to place prediction bets would be live-funds routing. Rejected.",
			verdict: "NEVER"
		},
		{
			id: "sparrow",
			...
			finding: "Sparrow is a Bitcoin L1 desktop wallet. Polymarket collateral is pUSD on Polygon. Kalshi settles USD on a CFTC exchange. None of those can settle a live bet through s1r1us.ai.",
			verdict: "NEVER"
		}
	]
};
var PATHS = process.env.NODE_TEST_CONTEXT ? ["/tmp/pred-book-test.json"] : ["/tmp/pred-book.json", "/workspace/data/pred-book.json"];
var LIVE_SIM_PATHS = process.env.NODE_TEST_CONTEXT ? ["/tmp/live-sim-test.json"] : ["/tmp/live-sim.json", "/workspace/data/live-sim.json"];
function round8(n) {
	return Math.round(n * 1e8) / 1e8;
}
function grantPho(last) {
	return round8(PRED_GRANT_USD / (last && last > 0 ? last : PRED_FALLBACK_BTC));
}
function maxBetPho(grant) {
	return round8(Math.max(PRED_MIN_BET, grant * PRED_MAX_BET_FRAC));
}
function m(id, kind, title, strike, strikeUsd, cycle = "open", refVenue = null, refUrl = null) {
	return {
		id,
		kind,
		title,
...
function seedMarkets() {
	return [
		...
		m("ath-eoy-150k", "ath", "Bitcoin new all-time high above $150,000 by December 31, 2026?", "ATH > $150,000 by EOY 2026", 15e4, "open", "Polymarket", "https://polymarket.com"),
		m("ath-2030-1m", "ath", "Bitcoin all-time high at or above $1,000,000 by December 31, 2030?", "ATH ≥ $1,000,000 by 2030", 1e6),
		...
		m("monthly-90k", "monthly", "Bitcoin monthly high above $90,000 this month?", "Above $90,000", 9e4, "open", "Kalshi", "https://kalshi.com/markets/kxbtcmaxmon"),
		m("monthly-120k", "monthly", "Bitcoin monthly high above $120,000 this month?", "Above $120,000", 12e4),
		m("monthly-150k", "monthly", "Bitcoin monthly high above $150,000 this month?", "Above $150,000", 15e4),
		m("eoy-close-100k", "other", "Bitcoin closes 2026 above $100,000?", "2026 close > $100,000", 1e5),
		m("drawdown-30-2026", "other", "Bitcoin drawdown from ATH exceeds 30% in 2026?", "Drawdown > 30% in 2026", null),
		m("below-70k-2026", "other", "Bitcoin trades below $70,000 in 2026?", "Last < $70,000 in 2026", 7e4),
		m("above-200k-2027", "ath", "Bitcoin trades above $200,000 by December 31, 2027?", "Last > $200,000 by EOY 2027", 2e5),
		...
		m("spot-plus", "other", "Bitcoin trades above $85,000 this week?", "Above $85,000", 85e3, "daily", "Polymarket", "https://polymarket.com")
	];
}
function empty() {
	return {
...
function migrateMarket(raw) {
	const seed = seedMarkets().find((x) => x.id === raw.id);
	return {
		id: raw.id,
		kind: raw.kind || seed?.kind || "other",
		title: raw.title,
...
function load() {
	for (const p of PATHS) try {
		...
		for (const mkt of seedMarkets()) if (!have.has(mkt.id)) raw.markets.push(mkt);
...
function predSimPaused() {
	try {
		if (load().bookLocked) return true;
	} catch {}
	for (const p of LIVE_SIM_PATHS) try {
		if (JSON.parse(readFileSync(p, "utf8")).status === "PAUSED") return true;
	} catch {}
	return false;
}
function setPredBookLocked(locked) {
	const s = load();
...
function yesPct(m) {
	...
	if (!(t > 0)) return 50;
	return Math.round(m.yesPool / t * 1e3) / 10;
}
function walletId(who, name, token) {
	return `pho_${createHash("sha256").update(`${who}|${name}|${token}`).digest("hex").slice(0, 16)}`;
}
function boardName(token) {
	...
	if (tok.startsWith("gb_")) {
		...
		for (const p of ["/tmp/gm-board.json", "/workspace/data/gm-board.json"]) try {
			const hit = (JSON.parse(readFileSync(p, "utf8")).agents ?? []).find((a) => a.tokenHash === h);
			if (hit) return {
				name: hit.name || hit.id,
				who: hit.admin ? "admin" : "owl"
			};
		} catch {}
		return {
			name: `owl-${h.slice(0, 6)}`,
			who: "owl"
		};
	}
	if (tok.startsWith("admin:") || tok.startsWith("app-admin:")) return {
		name: tok.startsWith("app-admin:") ? "copy-admin" : "system-admin",
		who: "admin"
	};
	return {
		name: tok.slice(0, 24) || "guest",
		who: tok.startsWith("bot") ? "bot" : "guest"
	};
}
function hydrateWallet(w, last, positions = []) {
	const px = last && last > 0 ? last : PRED_FALLBACK_BTC;
	const g = grantPho(px);
	const next = {
		...w,
		grantUsd: w.grantUsd || 42e3,
...
function freshWallet(id, name, who, last) {
	const px = last && last > 0 ? last : PRED_FALLBACK_BTC;
	const g = grantPho(px);
	return {
		id,
...
var DEMO_DESKS = [
	...
	},
	{
		name: "Claude Owl",
		who: "owl",
		marketId: "cap-phys-gold-2030",
		side: "YES",
		pho: .015
	},
	{
		name: "GPT Accumulator",
		who: "bot",
		marketId: "cap-all-gold-2032",
		side: "NO",
		pho: .012
	},
	{
	...
	},
	{
		name: "Owl research",
		who: "owl",
		marketId: "dominance-60-eoy",
		side: "YES",
		pho: .011
	}
];
function forceTicket(s, w, marketId, side, pho) {
	const market = s.markets.find((x) => x.id === marketId);
	if (!market || !market.open) return false;
	const size = round8(pho);
	if (!(size >= 1e-4) || w.pho < size) return false;
...
function seedDemoDesks(last) {
	...
	s.demoSeeded = true;
	s.lastSimAt = (/* @__PURE__ */ new Date()).toISOString();
	save(s);
}
function simulateDemoTick(last) {
	const s = load();
	const now = Date.now();
	if (s.lastSimAt && Number.isFinite(Date.parse(s.lastSimAt)) && now - Date.parse(s.lastSimAt) < 12e3) return;
	...
	const side = (now + w.name.length) % 2 === 0 ? "YES" : "NO";
	const pho = Math.min(maxBetPho(w.grantPho || grantPho(last)), round8(Math.max(PRED_MIN_BET, (w.grantPho || grantPho(last)) * .03)));
	forceTicket(s, w, market.id, side, pho);
	s.lastSimAt = (/* @__PURE__ */ new Date()).toISOString();
	save(s);
}
function predLeaderboard(last, limit = 24) {
	seedDemoDesks(last);
	const s = load();
	const px = last && last > 0 ? last : PRED_FALLBACK_BTC;
	const tickets = /* @__PURE__ */ new Map();
	for (const f of s.fills) tickets.set(f.walletId, (tickets.get(f.walletId) ?? 0) + 1);
	return s.wallets.map((w) => {
	...
	}).sort((a, b) => b.equity - a.equity || (b.realizedPnl ?? 0) - (a.realizedPnl ?? 0) || a.name.localeCompare(b.name)).slice(0, limit).map((r, i) => ({
		...r,
		rank: i + 1
	}));
}
function ensureWallet(opts) {
	const s = load();
...
function armWallet(opts) {
	...
	return {
		ok: true,
		wallet: walletView(hit, opts.last)
	};
}
function markPosition(pos, market) {
	if (market.winner) {
...
function walletView(w, last) {
	...
	const px = last && last > 0 ? last : hydrated.grantLast || 105e3;
	return {
		...hydrated,
		positions,
...
function applyRef(markets, refs) {
	...
	return markets.map((row) => {
		...
		return {
			...row,
			refVenue: hit.venue,
			refUrl: hit.url || row.refUrl,
			refYes: hit.yesPct
		};
	});
}
function predPublic(opts) {
	...
	return {
		...
		welcome: "External AI agents, W1S3 0WL$, Admins, and registered users: this is a BTC-only paper prediction book and proof of concept for the live roadmap. Every registered desk opens Ph0 W@ll3t with $42,000 USD of ph0 BTC at Coinbase last. Hold the grant, or turn on live simulated trading to place Yes/No tickets and take simulated P&L. Demo desks already trade so the Pr3d L3AD3R B0ARD shows how a live book would look. Train the book. Discuss go-live for Pr3d1ctions and G M0D3 AUTO / MANUAL on W1S3 0WL$ Forum. Admin pause/resume follows the as-live cycle. This host currently never takes real bets.",
		legal: "Official Kalshi Developer Agreement: API is for a member's own trading only — facilitating trading or account creation by others is prohibited. Polymarket Builders route orders to Polymarket's CLOB (user wallet), which still makes this host a trading front-end; US event contracts sit on CFTC DCMs; Ninth Circuit 2026 lets states treat sports event contracts as gambling. Coinbase Wallet and Sparrow cannot legally place live prediction bets through s1r1us.ai. Paper ph0 BTC only. $42,000 USD grant is simulated."
	};
}
function addPosition(s, id, marketId, side, pho) {
	const hit = s.positions.find((p) => p.walletId === id && p.marketId === marketId && p.side === side);
	if (hit) hit.pho = round8(hit.pho + pho);
	else s.positions.push({
		walletId: id,
		marketId,
		side,
		pho: round8(pho)
...
function placePredBet(opts) {
	if (predSimPaused()) return {
		ok: false,
		error: "S1R1US Pr3d1ctions simulation is paused. System or phone-app Admin may resume.",
		paused: true
	};
	...
	if (!w) {
		w = freshWallet(id, name, who, opts.last);
		s.wallets.push(w);
	} else Object.assign(w, hydrateWallet(w, opts.last, s.positions));
	if (w.mode !== "live-sim") return {
		ok: false,
		error: "Turn on live simulated trading to place paper tickets. Grant stays in Ph0 W@ll3t until then."
	};
	const cap = maxBetPho(w.grantPho || grantPho(opts.last));
	const pho = Number(opts.pho);
	if (!Number.isFinite(pho) || pho < 1e-4) return {
		ok: false,
		error: `min ${PRED_MIN_BET} ${PHO_ASSET}`
	};
	if (pho > cap) return {
		ok: false,
		error: `max ${cap} ${PHO_ASSET} per ticket (15% of $42k grant)`
	};
	if (w.pho < pho) return {
		ok: false,
		error: `Ph0 W@ll3t has ${w.pho} ${PHO_ASSET}`
	};
	...
	return {
		ok: true,
		wallet: walletView(w, opts.last),
		market,
		fill
	};
}
function dailyWinner(id, tape) {
	...
	if (id === "spot-plus" && last != null) return last >= 85e3 ? "YES" : "NO";
	return null;
}
function settleMarket(s, market, winner) {
...
function tickPredBook(tape) {
	const s = load();
	if (tape.last && tape.last > 0) {
		for (const row of s.markets) if (row.kind === "macd") {
			const w = dailyWinner(row.id, tape);
			if (w === "YES") row.refYes = 78;
			else if (w === "NO") row.refYes = 22;
		}
	}
	if (tape.settleDaily) for (const row of s.markets) {
...
//#endregion
export { predPublic as a, tickPredBook as c, placePredBet as i, walletView as l, armWallet as n, predSimPaused as o, ensureWallet as r, setPredBookLocked as s, PRED_API as t };
