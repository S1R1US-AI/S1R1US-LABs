...
import { r as DATA_FEEDS, s as fetchPredictionMarkets } from "./prediction-markets-BKVhDauB.mjs";
import { C as volumeRatio, S as volumeAvg24h, b as rsiWilder, c as atrLast, f as depthHeatmap, g as mergeHourly, h as macdLast, i as MAG7_TICKERS, l as bbPercentB, m as liqHeatmap, n as GOLD_TICKERS, o as PROXY_QUOTES, p as emaLast, r as MACRO_QUOTES, s as SILVER_TICKERS, t as FILING_CIKS, u as candlesToClosesOldestFirst, v as rsiMean, x as smaLast } from "./proxy-book-BG4V1YAe.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/sources-C2ZxZQmd.js
...
function scoreFeeds(snap) {
	...
	const rows = DATA_FEEDS.map((f) => {
		...
		switch (f.id) {
			...
			case "pred": return row(f.id, snap.predictionMarkets.length > 0, `${(snap.predictionMarkets ?? []).length} BTC markets (Polymarket/Kalshi, display only)`);
		}
	});
...
function maxBytesFor(url) {
	if (/gamma-api\.polymarket|elections\.kalshi/.test(url)) return 28e4;
...
function ttlFor(url) {
	if (/polymarket\.com|kalshi\.com/.test(url)) return 6e4;
...
async function fredCsv(ids, cosd) {
	...
	if (/^\s*<!DOCTYPE/i.test(text) || /<html/i.test(text.slice(0, 200))) throw new Error(`FRED ${ids} html wall`);
	return text;
}
async function fredSeries(id, name, years, yoy = false) {
	...
	try {
		...
		if (/^\s*<!DOCTYPE/i.test(txt) || /<html/i.test(txt.slice(0, 200))) throw new Error("FRED txt html wall");
...
function strategyTape(quoteRows = [], sparks = /* @__PURE__ */ new Map()) {
	const prev = snapCache?.value?.strategy;
	const live = new Map(quoteRows.map((q) => [q.symbol.toUpperCase(), q]));
	const products = STRATEGY_BOOK.map((meta) => {
		...
		const change6m = spark?.change6m ?? prevP?.change6m ?? (first && last != null && first > 0 ? (last - first) / first * 100 : null);
		return {
			...meta,
			last,
...
async function yahooChartSpark(symbol) {
	const res = (await getJson(`https://query2.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(symbol)}?range=6mo&interval=1wk`, 2200, { "User-Agent": "Mozilla/5.0 S1R1US-Lab/1.0" })).chart?.result?.[0];
	...
	for (let i = 0; i < ts.length; i++) {
		const v = closes[i];
		const t = ts[i];
		if (t != null && v != null && Number.isFinite(v)) points.push({
			t: t * 1e3,
			v: Number(v)
		});
	}
	...
	return {
		last,
		changePct: last != null && Number.isFinite(prevClose) && prevClose > 0 ? (last - prevClose) / prevClose * 100 : Number.isFinite(Number(res?.meta?.regularMarketChangePercent)) ? Number(res?.meta?.regularMarketChangePercent) : null,
		change6m,
		points
	};
}
async function strategySparks() {
	const out = /* @__PURE__ */ new Map();
	(await Promise.allSettled(STRATEGY_BOOK.map((m) => capLane(yahooChartSpark(m.symbol), 2200)))).forEach((r, i) => {
		if (r.status === "fulfilled" && (r.value.points.length > 2 || r.value.last != null)) out.set(STRATEGY_BOOK[i].symbol, r.value);
	});
	return out;
}
...
async function buildSnapshot() {
	try {
		const { ensureLiveSimScheduler } = await import("./live-sim.server-DykJKMX5.mjs");
		ensureLiveSimScheduler();
	} catch {}
	if (isTapeFrozen()) return snapCache?.value ?? readLastGood()?.snap ?? skeletonSnapshot();
	resetCycleStats();
	const t0 = Date.now();
	const errors = [];
...
async function assembleFill(t0, core) {
	try {
		...
		const sparks = stR.status === "fulfilled" ? stR.value : /* @__PURE__ */ new Map();
