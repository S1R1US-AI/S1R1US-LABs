import { g as looksLikeAdminX, v as preferredXAccountId } from "./x-admin-CALKyy-K.mjs";
import { n as getPglite, t as ensureDbReady } from "./db-CnQahlAD.mjs";
...
import { a as getBaseURL, c as getProtocol, d as resolveBaseURL, f as wildcardMatch, i as PACKAGE_VERSION, l as isDynamicBaseURLConfig, n as GROK_PROVIDERS, o as getHost, r as NATIVE_X_PROVIDER_ID, s as getOrigin, t as GENERIC_OAUTH_ERROR_CODES, u as isRequestLike } from "./providers-DoZIBobl.mjs";
...
//#region ../../workspace/node_modules/.nitro/vite/services/ssr/assets/server-D008iv7P.js
		...
		hooks: { after: [{
			...
			handler: createAuthMiddleware(async (ctx) => {
				...
				if (returned instanceof Headers) {
					...
					const { setCookie } = await import("./server-CvwbZsEi.mjs").then((n) => n.t);
...
var publicSite = Boolean((env$1("BETTER_AUTH_URL") ?? "").includes("s1r1us.ai"));
var grokClientId = env$1("GROK_AUTH_CLIENT_ID") ?? (publicSite ? void 0 : "grok_preview");
var grokClientSecret = env$1("GROK_AUTH_CLIENT_SECRET") ?? (publicSite ? void 0 : "8bcdb7fc5a33874ad933ca568918d5790388a0795e44c4d1dea691f801b17ec5");
var twitterClientId = env$1("TWITTER_CLIENT_ID") ?? env$1("X_CLIENT_ID");
var twitterClientSecret = env$1("TWITTER_CLIENT_SECRET") ?? env$1("X_CLIENT_SECRET");
var nativeXConfigured = Boolean(twitterClientId && twitterClientSecret);
/** Grok broker never on s1r1us.ai, even if leftover GROK_AUTH_* remain on the host. */
var useGrokBroker = !authDisabled && !publicSite && Boolean(grokClientId && grokClientSecret);
var authConfigured = !authDisabled && (publicSite ? nativeXConfigured : nativeXConfigured || Boolean(grokClientId && grokClientSecret));
...
var PUBLIC_SITE_HOSTS = ["s1r1us.ai", "www.s1r1us.ai"];
var PUBLIC_SITE_ORIGINS = PUBLIC_SITE_HOSTS.map((h) => `https://${h}`);
...
async function grokBrokerUserInfo(tokens) {
	const profile = {};
	if (tokens.idToken && tokens.idToken.split(".").length >= 2) try {
		const payload = JSON.parse(Buffer.from(tokens.idToken.split(".")[1], "base64url").toString("utf8"));
		if (payload && typeof payload === "object" && !Array.isArray(payload)) Object.assign(profile, payload);
	} catch {}
	if (tokens.accessToken) try {
		...
		if (res.ok) {
			...
			if (data && typeof data === "object" && !Array.isArray(data)) Object.assign(profile, data);
		}
	} catch {}
	const id = preferredXAccountId(profile) || (typeof profile.sub === "string" ? profile.sub.trim() : "") || (typeof profile.id === "string" ? profile.id.trim() : "");
	if (!id) return null;
	const handle = [
		profile.preferred_username,
		profile.username,
		profile.nickname
	].find((v) => typeof v === "string" && looksLikeAdminX(v)) ?? null;
	return {
		id,
		...
		image: typeof profile.picture === "string" ? profile.picture : void 0,
		emailVerified: false
	};
}
...
var grokOAuthPlugin = useGrokBroker ? genericOAuth({ config: GROK_PROVIDERS.map(({ providerId, idp }) => ({
	providerId,
...
var auth = betterAuth({
	...
	database,
	...!authDisabled && nativeXConfigured ? { socialProviders: { twitter: {
		...
		mapProfileToUser: (profile) => {
			...
			return {
				name: (typeof data?.username === "string" ? data.username.replace(/^@/, "") : "") || data?.name || data?.id || "x",
