...
import { c as grokRateLimit, l as grokUsage, s as assertSafePayload } from "./security-BwwT3rcO.mjs";
import { storedAdminName, verifyAccessToken, verifyDeskToken } from "./access.server-DuAgQxfH.mjs";
import { t as createServerRpc } from "./createServerRpc-CcvdN_gc.mjs";
import { n as guardedFetch } from "./net-guard-C4Si76ZP.mjs";
import { c as predAnalyst } from "./prediction-markets-DqQwubc4.mjs";
import { n as runBots, t as heliosCall } from "./signal-GwlzOT1d.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/grok-Csnbxjdu.js
...
async function gradeWithKey(apiKey, question) {
	const { getLiveSnapshot } = await import("./sources-mkg_ptE5.mjs");
...
var askHeliosByo = createServerFn({ method: "POST" }).validator((input) => input).handler(askHeliosByo_createServerFn_handler, async ({ data }) => {
	...
	const { looksLikeSecret } = await import("./security-BwwT3rcO.mjs").then((n) => n.m).then((n) => n.m);
	if (looksLikeSecret(data.xaiKey)) return {
		ok: false,
		error: "Secret rejected. Never paste Coinbase keys, seeds, or xprv."
	};
	const { inspectAgentInput } = await import("./agent-security-dJ5dmEbA.mjs").then((n) => n.r).then((n) => n.r);
