...
import { c as grokRateLimit, l as grokUsage, s as assertSafePayload } from "./security-D9Edw5IJ.mjs";
import { storedAdminName, verifyAccessToken, verifyDeskToken } from "./access.server-C0VcAINt.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/grok-DpDuLuaS.js
...
async function gradeWithKey(apiKey, question) {
	const { getLiveSnapshot } = await import("./sources-svDBMZmT.mjs");
...
var askHeliosByo = createServerFn({ method: "POST" }).validator((input) => input).handler(askHeliosByo_createServerFn_handler, async ({ data }) => {
	...
	const { looksLikeSecret } = await import("./security-D9Edw5IJ.mjs").then((n) => n.m).then((n) => n.m);
