...
//#region node_modules/.nitro/vite/services/ssr/assets/desk-rpc-CNdlwAV4.js
...
var fetchDesk = createServerFn({ method: "GET" }).validator((input) => input ?? {}).handler(fetchDesk_createServerFn_handler, async ({ data }) => {
	...
	const { isTapeFrozen } = await import("./tape-persist-Cbfygp4K.mjs");
...
var fetchTapeMeta = createServerFn({ method: "GET" }).handler(fetchTapeMeta_createServerFn_handler, async () => {
	const { lastGoodMeta } = await import("./tape-persist-Cbfygp4K.mjs");
	return lastGoodMeta();
});
...
var setTapeFreeze = createServerFn({ method: "POST" }).validator((input) => input).handler(setTapeFreeze_createServerFn_handler, async ({ data }) => {
	const { verifyAccessToken } = await import("./access.server-C0VcAINt.mjs");
	const { setTapeFrozen, lastGoodMeta } = await import("./tape-persist-Cbfygp4K.mjs");
