...
import { c as grokRateLimit, l as grokUsage, s as assertSafePayload } from "./security-Cm24ExbM.mjs";
import { storedAdminName, verifyAccessToken, verifyDeskToken } from "./access.server-B-78AuBV.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/grok-FBONo3vR.js
...
async function gradeWithKey(apiKey, question) {
	const { getLiveSnapshot } = await import("./sources-lzZLQHMx.mjs");
...
var askHeliosByo = createServerFn({ method: "POST" }).validator((input) => input).handler(askHeliosByo_createServerFn_handler, async ({ data }) => {
	...
	const { looksLikeSecret } = await import("./security-Cm24ExbM.mjs").then((n) => n.m).then((n) => n.m);
	if (looksLikeSecret(data.xaiKey)) return {
		ok: false,
		error: "Secret rejected. Never paste Coinbase keys, seeds, or xprv."
	};
	const { inspectAgentInput } = await import("./agent-security-j9HmdOss.mjs").then((n) => n.r).then((n) => n.r);
