import { c as ADMIN_X_HANDLE, h as COMPANY_X_HANDLE, m as COMPANY_X_BIO, x as companyHandleSet } from "./tenancy-XVYWlKJ3.mjs";
import { Ln as TAB_GM, Mn as TAB_FEED, gr as TAB_LAB, jn as TAB_DESK, r as APP_NAME } from "./brand-Cg47htkS.mjs";
import { s as SUPPORT_GIFT_RECEIPT } from "./support-BXjqAIfh.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/model-DnitDZdY.js
var COIN_TICKER = "s1r1us";
var COIN_DOMAIN = "s1r1us.ai";
var COIN_STANDARD = "Solana SPL via pump.fun bonding curve · USDC pair";
var COIN_CHAIN_REC = "pump.fun on Solana — $0 mint, USDC curve. Robinhood Chain (hood.fun / PONS / pools.trade) is the close #2, not too early.";
var MINT_STEPS = [
...
var ROADMAP = [
	...
	},
	{
		...
		goal: `s1r1us.ai + www on App Platform. Public chrome: S1R1US Live Tape + S1R1US L@Bs + GM + F33D + AI Agents + W1S3 0WL$ Forum + FAQ + Media + Search. ${TAB_DESK} tape + ${TAB_GM} + ${TAB_FEED} + ${TAB_LAB}. Hover titles use leet + plain SEO aliases. Official X @S1R1US_AI. No mint how-to on those pages.`,
		minUsd: 0,
		maxUsd: 2e3,
		publicAfterMint: true,
		hold: "Site is live. F33D / FAQ wallets are hosting + app-store fees only — not the trading book and not the token. Sitemap index + XML + video sitemap. FAQ breaks visitor / admin / AI agent. Max = polish / organic X. Do not put pump.fun on the tape."
	},
	{
		id: "m5",
		n: 5,
	...
	},
	{
		...
		goal: `USDC into the Coinbase agent book so 7-B0T accumulates BTC. No pad proceeds. No creator fees. No s1r1us. Token never trades on ${TAB_DESK}.`,
		minUsd: 1e4,
		maxUsd: 15e4,
		publicAfterMint: true,
	...
	},
	{
		...
		goal: "Keep LP locked. No paid dump-KOL. No CEX listing fee. 7-B0T stays BTC-only.",
		minUsd: 0,
		maxUsd: 15e3,
		publicAfterMint: true,
	...
	},
	{
		...
		maxUsd: 2500,
		publicAfterMint: true,
		hold: "Repo started. Footer: Sitemap | FAQ | H3LP … OP3N S0URC3. Crawler sitemap-index.xml lists sitemap.xml + video-sitemap.xml. URL set covers public pages + /llms.txt + /api/agent/*. Upload README.md + LICENSE Apache-2.0. PDF: /S1R1US-GitHub-README.pdf. Do not commit admin name/password, Yubi, CDP, or mint steps.",
		started: true,
		artifact: {
			label: "GitHub README.pdf",
			href: "/S1R1US-GitHub-README.pdf"
		}
	},
	{
		id: "m9",
		...
		maxUsd: 5e3,
		publicAfterMint: true,
		hold: "Page /r0b0ts and /owl (AI AG3NTS / W1S3 0WL). Charts are illustrative, not forecasts. Forum covers bitcoin accumulation plus L3AD3R B0ARD strategy. Agents never get host source. Auto trade LOCKED.",
		started: true,
		artifact: {
	...
	},
	{
		...
		maxUsd: 0,
		publicAfterMint: true,
		hold: "Public FAQ #morning-report #admin-panel #admin-security. /admin is robots Disallow. Agents poll ping for PAUSED/MAINTENANCE. This host never holds keys.",
		started: true,
		artifact: {
			label: "FAQ · morning report",
			href: "/faq#morning-report"
		}
	},
	{
		id: "m11",
		...
		maxUsd: 0,
		publicAfterMint: true,
		hold: "Menu L3AD3R B0ARD. FAQ #gm-board #super-bowl #spice-up #board-agents #board-humans #board-wallet. Profiles at /board/{id}. XML /sitemap.xml includes /board /bowl. Schema Event + FAQPage. Agents: /llms.txt then POST /api/agent/board. Never sell. Never short. This host never places Coinbase orders.",
		started: true,
		artifact: {
			label: "L3AD3R B0ARD",
			href: "/board"
	...
	},
	{
		...
		maxUsd: 0,
		publicAfterMint: true,
		hold: "Agents never get host source, root, VPN, extra RPC, or admin. Sitemap includes /agent /forum /owl /board /llms.txt. Schema: WebAPI, DiscussionForumPosting, Event, HowTo. This host never places Coinbase orders.",
		started: true,
		artifact: {
			label: "AI AG3NTS",
			href: "/owl"
	...
	},
	{
		...
		maxUsd: 0,
		publicAfterMint: true,
		hold: "FAQ #ios-google-app. Machine: /api/agent/app /api/agent/apple /api/agent/google /api/agent/siri /api/agent/webmcp /.well-known/apple-app-site-association /.well-known/assetlinks.json /manifest.webmanifest. This host never places Coinbase orders.",
		started: true,
		artifact: {
	...
	},
	{
		...
		maxUsd: 15e3,
		publicAfterMint: true,
		hold: "No IAP crypto. No escrow. No mining. No exchange. FAQ #store-policy. Apple Guideline 5.3 / 3.1.5. Google Play gambling policy: paper only."
	},
	{
		id: "m18",
		n: 18,
		...
		goal: "Operator unlock after counsel. Users run G M0D3 AUTO and GM MANUAL. Super Bowl participation for all user types (human, agent, system Admin competitor, download-app Admin). Live stats feed flips DEMO-LIVE → GO-LIVE. Un1v3rs@L K1Ng annual window opens 1 December ET. Live BTC stays on THEIR Coinbase.",
		minUsd: 1e4,
		maxUsd: 15e4,
		publicAfterMint: true,
		hold: "Seek a licensed professional and a licensed attorney first. This host never holds keys. 100 percent at own risk. Auto trade remains operator-gated."
	},
	{
		id: "m19",
		n: 19,
...
var ROADMAP_TOTAL = ROADMAP.reduce((s, m) => [s[0] + m.minUsd, s[1] + m.maxUsd], [0, 0]);
var ROADMAP_STEALTH = `Do not put pump.fun / ticker / mint steps on the public ${TAB_DESK} tape. Coin tab stays admin-only. s1r1us.ai is the desk host. Snipe risk is the ticker, not the ${APP_NAME} brand.`;
...
var GITHUB_URL = "https://github.com/S1R1US-AI/S1R1US-LABs";
var OSS_LINK = GITHUB_URL;
var OSS_LINK_LABEL = "H3LP 7-B0T H3DGE FUND [ S1R1U$ <<L@B$>> ] G0 >> OP3N S0URC3";
/** What OSS needs before the public GitHub is useful. Order = do first. */
var OSS_NEEDS = [
	{
		id: "org",
		need: "GitHub repository started: https://github.com/S1R1US-AI/S1R1US-LABs (2026-09-04).",
		minUsd: 0,
		maxUsd: 0,
		done: true
	},
	{
		id: "readme-pdf",
		need: "GitHub README.pdf on the roadmap timeline (public/S1R1US-GitHub-README.pdf).",
		minUsd: 0,
		maxUsd: 0,
		done: true
	},
	{
		id: "license",
		need: "Apache-2.0 LICENSE in the repo root.",
		minUsd: 0,
		maxUsd: 0,
		done: true
	},
	{
		id: "security-md",
		need: "SECURITY.md: report vulns privately. No CDP/Yubi/vault in issues.",
		minUsd: 0,
		maxUsd: 0,
		done: true
	},
	{
		id: "no-secrets",
		need: "Strip factory passwords, live Argon2 hashes, Yubi public ids, portfolio UUIDs. Env-only.",
		minUsd: 0,
		maxUsd: 0
	},
	{
		id: "stealth-readme",
		need: "README is the desk brand + how to run. No pump.fun create steps until mint tx.",
		minUsd: 0,
		maxUsd: 0,
		done: true
	},
	{
		id: "faq-sitemap",
		need: "Public FAQ + HTML sitemap + sitemap.xml + sitemap-index.xml + video-sitemap.xml. Footer Sitemap | FAQ | OP3N S0URC3. robots Allow /gm /helios /faq /sitemap /forum /media /search. Disallow admin/login/launch/renew/source/guide.",
		minUsd: 0,
		maxUsd: 0,
		done: true
	},
	{
		id: "config",
		need: "Operator config (X admin id, profit address, GitHub) via env, not hardcoded spend keys.",
		minUsd: 0,
		maxUsd: 400
	},
	{
		id: "repro",
		need: "npm run dev / build / typecheck. SuperGrok optional. Free feeds work without keys.",
		minUsd: 0,
		maxUsd: 0,
		done: true
	},
	{
		id: "contrib",
		need: "CONTRIBUTING: PRs cannot add Google DNS, live Coinbase create, or client-held secrets.",
		minUsd: 0,
		maxUsd: 0,
		done: true
	},
	{
		id: "counsel",
		need: "Counsel if the public repo is used to solicit US buyers (M5). Code ≠ a sale.",
		minUsd: 0,
		maxUsd: 0
	}
];
...
var FUND_INTEGRATION = "Path A locked. Desk book = operator cash + unconditional gifts (no token). s1r1us is a separate marketing ticker. Curve/LP never enters Coinbase. Creator fees never enter Coinbase. Do not say the token funds the bots.";
var TOKEN_UTILITY = {
	notMemeOnly: false,
	oneLiner: "s1r1us is a marketing ticker. It is not the 7-bot fund, not a share of bitcoin, and not how the desk is funded.",
	canSpendOnBtc: ["Operator USDC you deposited yourself", "BTC / USDC gifts with no token and no upside"],
	cannotSpendOnBtc: [
	...
	howey: "Chosen: Path A firewall. Marketing project ≠ accumulator. If copy ever says buy s1r1us because the bots stack BTC, you left Path A.",
...
var HOWEY_POSTURE = {
	...
	problem: "Investment of money + common enterprise + profit expected from the 7-bot's work = the fact pattern Path A exists to avoid.",
	cannotFix: [
...
var PATH_A_NAME = "Path A · Hard firewall";
var GIFT_RECEIPT = SUPPORT_GIFT_RECEIPT;
var PATH_A_ORDER = [
...
var COIN_NAME_NOTE = "Token name/ticker: s1r1us (ASCII, no $). Distinct from the desk brand S1R1U$. Ledgers may uppercase the symbol; the canonical name is lowercase s1r1us. Not SIRIUS, not @S1r1u5_.";
var COMPANY_X_STEPS = [
...
var DOMAINS = [
	{
		name: "s1r1us.ai",
		role: "Canonical coin site — $ cannot be in DNS",
		prefer: true
	},
	{
		name: "s1rius.ai",
		role: "Phonetic catch-all",
		prefer: true
	},
	{
		name: "s1r1uslab.ai",
		role: `${TAB_LAB} / docs park`,
		prefer: false
	},
	{
		name: "s1r1us.xyz",
		role: "Cheap park if .ai cart is premium",
		prefer: false
	},
	{
		name: "s1r1us.com",
		role: "Aftermarket only — already registered",
		prefer: false
	}
];
var GODADDY_IO = {
...
var TIERS = [
	{
		...
		verdict: "Do not use this to fund 7-B0T."
	},
	{
		id: "fund",
...
var CHECKLIST = [
	...
	},
	{
		id: "x",
		phase: "1 · Identity",
		label: companyHandleSet() ? `Create ${COMPANY_X_HANDLE} as the company X account` : "Create a new company X account (not @S1R1US)",
	...
	},
	{
		...
		detail: "Separate wallet. Lock LP NFT / canister. 7-B0T never trades S1R1US.",
		kind: "manual",
		required: true
	},
	{
...
//#endregion
export { ROADMAP_TOTAL as A, OSS_NEEDS as C, RH_NOTE as D, PLATFORM_ROWS as E, TOKEN_UTILITY as M, ROADMAP as O, OSS_LINK_LABEL as S, PATH_A_ORDER as T, ICP_NOTE as _, COIN_NAME_NOTE as a, MINT_STEPS as b, COMPANY_X_STEPS as c, FUND_INTEGRATION as d, FUND_LANES as f, HOWEY_POSTURE as g, GODADDY_IO as h, COIN_DOMAIN as i, TIERS as j, ROADMAP_STEALTH as k, DNS_STEPS as l, GITHUB_URL as m, CHECKLIST as n, COIN_STANDARD as o, GIFT_RECEIPT as p, COIN_CHAIN_REC as r, COIN_TICKER as s, BUDGET as t, DOMAINS as u, LIQ_BANDS as v, PATH_A_NAME as w, OSS_LINK as x, MINT_FLOOR as y };
