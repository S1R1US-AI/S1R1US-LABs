import { _ as getSql, f as isAdminXProvider, p as looksLikeAdminX } from "./x-admin-Dt29NgIk.mjs";
import { f as looksLikeSecret } from "./security-CdTEEtPf.mjs";
import { r as argon2id } from "../_libs/noble__hashes.mjs";
import { createHash, createHmac, randomBytes, timingSafeEqual } from "node:crypto";
import { AsyncLocalStorage } from "node:async_hooks";
//#region node_modules/.nitro/vite/services/ssr/assets/access.server-zj_kFVQY.js
...
async function sessionIsAdminX(userId) {
	...
	try {
		const accounts = await (await getSql())`
...
function idTokenSnowflakeIsAdmin(token) {
	if (!token || token.split(".").length < 2) return false;
	try {
		const payload = JSON.parse(Buffer.from(token.split(".")[1], "base64url").toString("utf8"));
		return [payload.sub, payload.user_id].some((v) => typeof v === "string" && v.trim() === "2093335535146131456");
	} catch {
		return false;
	}
}
