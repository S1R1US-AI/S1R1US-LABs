import { n as GO_LIVE_DEADLINE } from "./go-live-CaLXlH4n.mjs";
import { n as checkpointLabel } from "./checkpoint-Bsdvrfn5.mjs";
import { i as liveSimNote, n as LIVE_SIM_SEO, r as LIVE_SIM_TZ, t as LIVE_SIM_NAME } from "./live-sim-BsobJS8V.mjs";
import { i as setTapeFrozen, r as readLastGood, t as isTapeFrozen } from "./tape-persist-CHN2yE-V.mjs";
import { r as stampGoLiveNotice } from "./go-live-notices-y5KTHj_G.mjs";
import { writePulse } from "./practice-pulse-DsIXMyE2.mjs";
import { c as tickPredBook } from "./pred-book-3k6waxCi.mjs";
import { existsSync, mkdirSync, readFileSync, writeFileSync } from "node:fs";
//#region node_modules/.nitro/vite/services/ssr/assets/live-sim.server-DsPwAQh9.js
/**
...
function tickPredFromTape(settleDaily) {
	try {
...
function killStrayPractice() {
	try {
		...
		writeFileSync(PRACTICE_KILL, JSON.stringify({
			...
			note: "Old AUTO 24h practice and extra paper ticks stay off. Live-sim is the only as-live cycle."
		}, null, 2));
	} catch {}
	writePulse({
		at: (/* @__PURE__ */ new Date()).toISOString(),
...
function liveSimPublic() {
	const s = load();
	const w = nextWindow();
	const synced = s.checkpoint === 68 && s.conflict === null;
	return {
		name: LIVE_SIM_NAME,
		seo: LIVE_SIM_SEO,
		status: s.status,
...
function testLiveSimCheckpoint() {
	...
	const notes = [];
	if (pub.checkpoint !== 68) notes.push("checkpoint desync");
