...
import { c as grokRateLimit, l as grokUsage, s as assertSafePayload } from "./security-BGs2AzWv.mjs";
import { storedAdminName, verifyAccessToken, verifyDeskToken } from "./access.server-BhqnGgjj.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/grok-DvNezVt-.js
...
async function gradeWithKey(apiKey, question) {
	const { getLiveSnapshot } = await import("./sources-Dk79xI8O.mjs");
...
var askHeliosByo = createServerFn({ method: "POST" }).validator((input) => input).handler(askHeliosByo_createServerFn_handler, async ({ data }) => {
	...
	if (!await getSessionUser(data.bearer)) return {
		ok: false,
	...
	const { looksLikeSecret } = await import("./security-BGs2AzWv.mjs").then((n) => n.m).then((n) => n.m);
	if (looksLikeSecret(data.xaiKey)) return {
		ok: false,
		error: "Secret rejected. Never paste Coinbase keys, seeds, or xprv."
	};
	const { inspectAgentInput } = await import("./agent-security-fYS4prvN.mjs").then((n) => n.r).then((n) => n.r);
